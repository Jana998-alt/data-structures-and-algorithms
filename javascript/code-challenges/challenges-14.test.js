...
const toTitleCase = (arr) => {
  let newArr = arr.map(element => {
    return element.substr(0, 1).toUpperCase() + element.substr(1, element.length);
  })
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let finalStr = '';
  let index = 0;
  let luke = arr.find(element => { element.name == 'Luke Skywalker' ; return element});
  let lukesMass = luke.mass;
  arr.forEach(element => {
    if (Number(element.mass) > Number(lukesMass)) {
      if (index == 0) {
        finalStr = finalStr + element.name;
        index = 1;
      }
      else {
        finalStr = `${finalStr} - ${element.name}`
...
const sortBy = (property, arr) => {
  let hi =arr.sort((el1,el2) => {
    return (Number(el1.property)-Number(el2.property));
  })
  return hi;
};
...
const detectTicTacToeWin = (board) => {
  ...
  //       }
  //       else if(board[i][i] == board[i][i] ==board[i][i]){
  //         return true;
  //       }
