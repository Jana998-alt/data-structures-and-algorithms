...
const sortByChildren = (charArray) => {
  ...
  characters.sort((obj1,obj2)=>{
    if(obj1.children.length < obj2.children.length){ return 1}
    if(obj2.children.length < obj1.children.length){ return -1}
    if(obj1.children.length === obj1.children.length){ 
      if(obj1.children.name.toUpperCase() < obj2.children.name.toUpperCase()){ return 1}
      if(obj2.children.name.toUpperCase() < obj1.children.name.toUpperCase()){ return -1}
    }
  })
};
...
const containsW = (str) => {
  let lettersArray = 'sw'.split('');
  let found =lettersArray.find(arr =>arr == 'w');
...
const isNum = (input) => {
  let inputString = toString(input);
  let inputArray = inputString.split('');
  for(let i=0;i<inputArray.length;i++){
    if(typeof(parseInt(inputArray[i])) == 'number'){
      return true;
      break;
    }
    else {
