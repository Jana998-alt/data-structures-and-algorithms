...
function transformToLis(obj){
  ...
  let keyValuePairsArr = Object.entries(obj);
  keyValuePairsArr.forEach((element,index) => {
    keyValuePairsArr[index] = `<li>${element[0]}: ${element[1]}</li>`;
  });
...
const count = (target, input) => {
  let final = 0;
  final= input.reduce((finalAcc,currentVal) => {
    let smallAcc = currentVal.reduce((acc,cur) => {
      if(cur==target){
        acc = acc+1;
      }
      return acc;
    },0)
    finalAcc = finalAcc + smallAcc;
    return finalAcc;
  },0)
  return final;
};
...
const totalSum = (input) => {
  ...
  let total = input.reduce((acc,cur)=>{
    smallTotal = cur.reduce((acc2,cur2)=>{
      acc2 = acc2+cur2;
      return acc2;
    },0)
    acc = acc+smallTotal;
...
input.forEach(element => {
  filteredArray.push(element.filter(element=>{return (typeof element) == 'number' && element%5 == 0}));
  ...
  filteredArray.forEach((element,index)=>{
    element.forEach((element2,index2) =>{
      filteredArray[index][index2] = Math.pow(2, element2);
    })
  })
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach((element,index) => {
    if (element.gender == 'female' || element.gender == 'male'){
      if (added != 0)
      {returned = `${returned} and ${element.name}`}
      else {returned = `${element.name}`; added = 1}
    }
  })
 return returned;
};
