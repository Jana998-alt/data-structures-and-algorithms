...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsArr.sort((el1,el2)=>{
    if (parseInt(el1.height) < parseInt(el2.height)){
      return 1;
    }
    else if (parseInt(el1.height) > parseInt(el2.height)){
      return -1;
    }
    return 0;
  })
...
const howMuchPencil = (str) => {
  ...
  for(let i=0;i<str.length;i++){
    ...
    result.push(newStr.slice(1,newstr.length))
